...
def processOnePerson(filename,lookback=3,dropout_value=0.5,learning_rate=1e-1,epoch=10,layer_num=4):
    #initialize adam
    print("i am now process ",filename)
    size_of_batch = 128
    ...
    model = Sequential()
    model.add(SimpleRNN(layer_num,input_shape=(lookback,85)))
    model.add(Dense(1))
    ...
    for i in range(len(trainPredict)):
        trainPredict[i] = trainPredict[i][0]
    mse = mean_squared_error(trainPredict,preLab)
    coe = pearsonr(trainPredict,preLab)[0]
    if(coe>bestcoe):
        bestcoe = coe
        bestmse = mse
